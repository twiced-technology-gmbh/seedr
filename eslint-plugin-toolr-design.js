...
function loadManifest() {
  // Try the standard export first (works when published from npm)
  try {
    const require = createRequire(import.meta.url)
    return require('@toolr/ui-design/manifest')
  } catch { /* continue */ }

  // With pnpm file: references, ai-manifest.json may not be hardlinked into the
  // store (it must be in the package's `files` field). Follow the file: reference
  // from the consumer's package.json to find the source directory.
  // Try both monorepo root (apps/web/) and direct cwd (turbo runs from package dir).
  const pluginDir = dirname(fileURLToPath(import.meta.url))
  for (const pkgPath of [
    join(pluginDir, 'apps/web/package.json'),
    ...
    try {
      ...
      const ref = pkg.dependencies?.['@toolr/ui-design'] ?? ''
      if (ref.startsWith('file:')) {
        const sourceDir = join(dirname(pkgPath), ref.slice(5))
        return JSON.parse(readFileSync(join(sourceDir, 'ai-manifest.json'), 'utf8'))
      }
    } catch { /* continue */ }
  }
...
const ALLOWED_SPACING_VALUES = new Set([
  // Component spacing (fine-grained)
  '0', '0.5', '1', '1.5', '2', '3', '4', '5', '6',
  // Layout spacing (standard Tailwind integers on the 4px grid)
