...
export function Detail() {
  ...
  const installCommand = `npx seedr add ${item.slug}`;
      ...
      <div>
        ...
        <div className="space-y-4">
          <CodeBlock
            ...
            code={`npx seedr add ${item.slug} --agents all`}
          />
          <CodeBlock
            label="Install for specific tool"
            code={`npx seedr add ${item.slug} --agents claude`}
          />
          <CodeBlock
            label="Non-interactive (CI/scripts)"
            code={`npx seedr add ${item.slug} --agents all --scope project --method symlink --yes`}
          />
        </div>
      </div>
    </div>
